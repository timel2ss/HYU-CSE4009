...
int remainderPower2(int x, int n) {
    int sign, mask, remainder;
    sign = x >> 31 << n;
    mask = (1 << n) + (~0);
    remainder = x & mask;
    return (sign & (!remainder + ~(0))) | remainder;
}
/* 
 ...
 */
int satMul2(int x) {
  int mul2, isOverflowed, isNegative, TMin;
  mul2 = x << 1;
  isOverflowed = (mul2 ^ x) >> 31;
  TMin = 1 << 31;
  isNegative = mul2 >> 31;
  return (~isOverflowed & mul2) | (isOverflowed & (TMin ^ isNegative));
}
/* 
